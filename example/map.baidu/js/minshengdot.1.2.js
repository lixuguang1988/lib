...
BankDotOverlay.prototype.show = function(){
	//关闭之前激活状态的银行网点覆盖物
     ...
     if(typeof activePanel === 'function'){
     	activePanel(this._index);      	
     }
};

BankDotOverlay.prototype.recenter = function(){
...
BankDotOverlay.prototype.hide = function(){    
     ...
     //激活panel列表对应项
     if(typeof unactivePanel === 'function'){
     	unactivePanel(this._index);      	
     }     
};
...
/**
 * 初始化地图 
 * @param {string|Object} container 地图显示的元素  ["xmap", Dom Node]
 * @param {string} url 服务器端数据来源
 * @param {Object} cityname 初始化的城市名
 */
MapX.initialize =  function(container, url, cityname){
	var map;
	
	if(arguments.length < 1){
		throw new Error("该函数必须有个参数");
	}
...
MapX.renderOverlay = function(){
	var msdotPanel = document.getElementById('mapanel'), /*民生银行网点列表面板*/
		bankdotOverlay = [],
		index = 0; /*存储银行网点的覆盖物*/
	...
	for(var i = 0; i < msdot.length; i++){
		...
		var bankdot = new BankDotOverlay(this.map, point, msdot[i].title, i, {width:34,height:44});
...
MapX.getData =  function(){
	...
	this.xhr = $.ajax({
		url : _.url,
		type : "POST",
		data : {minLng : _.min.lng , minLat : _.min.lat, maxLng : _.max.lng , maxLat : _.max.lat},
		dataType : "json"
	}).done(function(data){
		console.log(data);
